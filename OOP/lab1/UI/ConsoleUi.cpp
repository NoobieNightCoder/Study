...
void ConsoleUi::printError(const string& errorMessage) const {
    io.printError(errorMessage);
}
...
vector<Point> ConsoleUi::readPolygonVertices() const {
    vector<Point> vertices = io.readPolygonVertices();
    return vertices;
}
