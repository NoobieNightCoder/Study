    ...
    T readValue(const std::string& prompt) const {
        ...
        while (!isValid) {
            ...
            if (isValid) {
                ...
                printError(InputException::Messages::InvalidInput);
                std::cout << prompt;
            }
        }
...
public:
    ...
    void printError(std::string errorMessage) const override;
    ...
    std::vector<Point> readPolygonVertices() const override;
};
