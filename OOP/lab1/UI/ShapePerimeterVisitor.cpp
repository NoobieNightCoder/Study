...
void ShapePerimeterVisitor::visitPolygon(const Polygon& polygon) {
    ostringstream text;
    text << polygon.getName() << " (" << ShapeTextCatalog::getPolygonLabel() << ")";
    text << ShapeTextCatalog::getSeparator() << ShapeTextCatalog::getPerimeterLabel() << polygon.getPerimeter();
    line = text.str();
}
