...
Student* deleteStudent(Student* students, int studentIndex, int* numOfStudents) {
    if(studentIndex < *numOfStudents) {
        if(*numOfStudents > 1) {
            swapStudents(&students[studentIndex], &students[*numOfStudents - 1]);
            *numOfStudents -= 1;
...
void defineAvgGradeOfStudents(Student** students, int numOfStudents) {
    for(int index = 0; index < numOfStudents; index++) {
        ...
        for(int i = 0; i < GRADES; i++)
            avgGrade += (*students)[index].grade[i];
        avgGrade /= GRADES;
        (*students)[index].averageGrade = avgGrade;
    }
}

void expulsionBadStudents(Student** students, int* numOfStudents) {
    for(int index = 0; index < *numOfStudents; index++) {
        if((*students)[index].type == D) {
            int counter = 0;
            for(int i = 0; i < GRADES; i++) {
                if((*students)[index].grade[i] == D)
                    counter++;
            }
            if(counter > 2)
                *students = deleteStudent(*students, index, numOfStudents);
        }
    }
}
